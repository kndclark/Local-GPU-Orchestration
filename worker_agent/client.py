...
class WorkerClient:
    ...
    async def register_node(self, hostname: str, total_vram_mb: int, gpu_count: int, supported_workloads: list[str]) -> bool:
        if not self.stub:
            self.connect()
    ...
    async def send_heartbeat(self, free_vram_mb: int, temp_c: float, util_percent: float, active_jobs: list[str]) -> bool:
        if not self.stub:
            self.connect()
    ...
    async def update_job_status(self, job_id: str, status: str, error_message: str = "") -> bool:
        if not self.stub:
            self.connect()
        req = orchestrator_pb2.JobStatusUpdate(job_id=job_id, node_id=self.node_id, status=status, error_message=error_message)
        try:
