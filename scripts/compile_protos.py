...
def main():
    ...
    # Generate the stubs directly into the existing packages (`control_plane` and `worker_agent`).
    # This avoids the need to define and manage a separate shared package in pyproject.toml
    # while keeping each component self-contained.
    ...
    for out_dir in out_dirs:
        ...
        if os.path.exists(grpc_file):
            with open(grpc_file, 'r') as f:
                content = f.read()
            content = content.replace('import orchestrator_pb2 as orchestrator__pb2', 'from . import orchestrator_pb2 as orchestrator__pb2')
