...
def test_grafana_json_datasource_jobs():
    """
    TDD Approach: Verify that Grafana can successfully proxy a query through the JSON datasource
    and return the jobs for a given node.
    """
        ...
        "queries": [
            {
                ...
                "type": "json",
                "source": "url",
                "url": "http://host.docker.internal:8080/api/v1/nodes/KhamuDeckOLED/jobs",
                "format": "table"
            }
        ],
        "from": "now-1h",
    ...
    assert grafana_resp.status_code == 200, f"Grafana returned error: {grafana_resp.text}"
    ...
    import json
    print("Grafana result_A:", json.dumps(result_A, indent=2))
